...
//
#ifndef AGPACK_V1_TYPE_MAP_HPP
#define AGPACK_V1_TYPE_MAP_HPP

#include "agpack/v1/adaptor/map_decl.hpp"
...
struct as<
    ...
    type::assoc_vector<K, V, Compare, Alloc> operator()(agpack::object const& o) const {
...
struct convert<type::assoc_vector<K, V, Compare, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, type::assoc_vector<K, V, Compare, Alloc>& v) const {
        if (o.type != agpack::type::MAP) { throw agpack::type_error(); }
...
struct pack<type::assoc_vector<K, V, Compare, Alloc> > {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const type::assoc_vector<K, V, Compare, Alloc>& v) const {
...
struct object_with_zone<type::assoc_vector<K, V, Compare, Alloc> > {
    void operator()(agpack::object::with_zone& o, const type::assoc_vector<K, V, Compare, Alloc>& v) const {
...
struct as<
    ...
    typename std::enable_if<agpack::has_as<K>::value || agpack::has_as<V>::value>::type> {
    std::map<K, V, Compare, Alloc> operator()(agpack::object const& o) const {
        if (o.type != agpack::type::MAP) { throw agpack::type_error(); }
        agpack::object_kv* p(o.via.map.ptr);
        agpack::object_kv* const pend(o.via.map.ptr + o.via.map.size);
...
struct convert<std::map<K, V, Compare, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, std::map<K, V, Compare, Alloc>& v) const {
        if (o.type != agpack::type::MAP) { throw agpack::type_error(); }
        agpack::object_kv* p(o.via.map.ptr);
        agpack::object_kv* const pend(o.via.map.ptr + o.via.map.size);
...
struct pack<std::map<K, V, Compare, Alloc> > {
    template <typename Stream>
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::map<K, V, Compare, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
        o.pack_map(size);
...
struct object_with_zone<std::map<K, V, Compare, Alloc> > {
    void operator()(agpack::object::with_zone& o, const std::map<K, V, Compare, Alloc>& v) const {
        o.type = agpack::type::MAP;
        if (v.empty()) {
            o.via.map.ptr  = AGPACK_NULLPTR;
            o.via.map.size = 0;
...
struct as<
    ...
    typename std::enable_if<agpack::has_as<K>::value || agpack::has_as<V>::value>::type> {
    std::multimap<K, V, Compare, Alloc> operator()(agpack::object const& o) const {
        if (o.type != agpack::type::MAP) { throw agpack::type_error(); }
        agpack::object_kv* p(o.via.map.ptr);
        agpack::object_kv* const pend(o.via.map.ptr + o.via.map.size);
...
struct convert<std::multimap<K, V, Compare, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, std::multimap<K, V, Compare, Alloc>& v) const {
        if (o.type != agpack::type::MAP) { throw agpack::type_error(); }
        agpack::object_kv* p(o.via.map.ptr);
        agpack::object_kv* const pend(o.via.map.ptr + o.via.map.size);
...
struct pack<std::multimap<K, V, Compare, Alloc> > {
    template <typename Stream>
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::multimap<K, V, Compare, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
        o.pack_map(size);
...
struct object_with_zone<std::multimap<K, V, Compare, Alloc> > {
    void operator()(agpack::object::with_zone& o, const std::multimap<K, V, Compare, Alloc>& v) const {
        o.type = agpack::type::MAP;
        if (v.empty()) {
            o.via.map.ptr  = AGPACK_NULLPTR;
            o.via.map.size = 0;
