...
//
#ifndef AGPACK_V1_TYPE_EXT_HPP
#define AGPACK_V1_TYPE_EXT_HPP

#include "agpack/v1/adaptor/ext_decl.hpp"
...
public:
    ...
    ext(int8_t t, const char* p, uint32_t s) {
        agpack::detail::check_container_size_for_ext<sizeof(std::size_t)>(s);
...
template <>
struct convert<agpack::type::ext> {
    agpack::object const& operator()(agpack::object const& o, agpack::type::ext& v) const {
        if(o.type != agpack::type::EXT) {
            throw agpack::type_error();
        }
        v = agpack::type::ext(o.via.ext.type(), o.via.ext.data(), o.via.ext.size);
        return o;
    }
};

template <>
struct pack<agpack::type::ext> {
    template <typename Stream>
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const agpack::type::ext& v) const {
...
template <>
struct object_with_zone<agpack::type::ext> {
    void operator()(agpack::object::with_zone& o, const agpack::type::ext& v) const {
        // size limit has already been checked at ext's constructor
        uint32_t size = v.size();
        o.type = agpack::type::EXT;
        char* ptr = static_cast<char*>(o.zone.allocate_align(size + 1, AGPACK_ZONE_ALIGNOF(char)));
        o.via.ext.ptr = ptr;
        o.via.ext.size = size;
...
private:
    const char* m_ptr;
    uint32_t m_size;
    friend struct adaptor::object<agpack::type::ext_ref>;
};
...
template <>
struct convert<agpack::type::ext_ref> {
    agpack::object const& operator()(agpack::object const& o, agpack::type::ext_ref& v) const {
        if(o.type != agpack::type::EXT) { throw agpack::type_error(); }
        v = agpack::type::ext_ref(o.via.ext.ptr, o.via.ext.size + 1);
        return o;
    }
};

template <>
struct pack<agpack::type::ext_ref> {
    template <typename Stream>
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const agpack::type::ext_ref& v) const {
        // size limit has already been checked at ext_ref's constructor
        uint32_t size = v.size();
        o.pack_ext(size, v.type());
        o.pack_ext_body(v.data(), size);
...
template <>
struct object<agpack::type::ext_ref> {
    void operator()(agpack::object& o, const agpack::type::ext_ref& v) const {
        // size limit has already been checked at ext_ref's constructor
        uint32_t size = v.size();
        o.type = agpack::type::EXT;
...
template <>
struct object_with_zone<agpack::type::ext_ref> {
    void operator()(agpack::object::with_zone& o, const agpack::type::ext_ref& v) const {
