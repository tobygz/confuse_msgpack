...
//
#ifndef AGPACK_V1_TYPE_VECTOR_HPP
#define AGPACK_V1_TYPE_VECTOR_HPP
...
struct as<std::vector<T, Alloc>, typename std::enable_if<agpack::has_as<T>::value>::type> {
    std::vector<T, Alloc> operator()(const agpack::object& o) const {
...
struct convert<std::vector<T, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, std::vector<T, Alloc>& v) const {
        if (o.type != agpack::type::ARRAY) { throw agpack::type_error(); }
...
struct pack<std::vector<T, Alloc> > {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::vector<T, Alloc>& v) const {
...
struct object_with_zone<std::vector<T, Alloc> > {
    void operator()(agpack::object::with_zone& o, const std::vector<T, Alloc>& v) const {
...
#endif // AGPACK_V1_TYPE_VECTOR_HPP
