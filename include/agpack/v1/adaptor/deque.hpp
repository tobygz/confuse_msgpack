...
//
#ifndef AGPACK_V1_TYPE_DEQUE_HPP
#define AGPACK_V1_TYPE_DEQUE_HPP
...
struct as<std::deque<T, Alloc>, typename std::enable_if<agpack::has_as<T>::value>::type> {
    std::deque<T, Alloc> operator()(const agpack::object& o) const {
...
struct convert<std::deque<T, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, std::deque<T, Alloc>& v) const {
        if(o.type != agpack::type::ARRAY) { throw agpack::type_error(); }
...
struct pack<std::deque<T, Alloc> > {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::deque<T, Alloc>& v) const {
...
struct object_with_zone<std::deque<T, Alloc> > {
    void operator()(agpack::object::with_zone& o, const std::deque<T, Alloc>& v) const {
...
#endif // AGPACK_V1_TYPE_DEQUE_HPP
