...
#ifndef AGPACK_V1_TYPE_CPP11_CHRONO_HPP
#define AGPACK_V1_TYPE_CPP11_CHRONO_HPP
...
struct as<std::chrono::system_clock::time_point> {
    typename std::chrono::system_clock::time_point operator()(agpack::object const& o) const {
        if(o.type != agpack::type::EXT) { throw agpack::type_error(); }
        if(o.via.ext.type() != -1) { throw agpack::type_error(); }
        std::chrono::system_clock::time_point tp;
        switch(o.via.ext.size) {
        case 4: {
            uint32_t sec;
            _agpack_load32(uint32_t, o.via.ext.data(), &sec);
            tp += std::chrono::seconds(sec);
        } break;
        case 8: {
            uint64_t value;
            _agpack_load64(uint64_t, o.via.ext.data(), &value);
        ...
        case 12: {
            uint32_t nanosec;
            _agpack_load32(uint32_t, o.via.ext.data(), &nanosec);
            int64_t sec;
            _agpack_load64(int64_t, o.via.ext.data() + 4, &sec);
            tp += std::chrono::duration_cast<std::chrono::system_clock::duration>(
                std::chrono::nanoseconds(nanosec));
            tp += std::chrono::seconds(sec);
        } break;
...
struct convert<std::chrono::system_clock::time_point> {
    agpack::object const& operator()(agpack::object const& o, std::chrono::system_clock::time_point& v) const {
        if(o.type != agpack::type::EXT) { throw agpack::type_error(); }
        if(o.via.ext.type() != -1) { throw agpack::type_error(); }
        std::chrono::system_clock::time_point tp;
        switch(o.via.ext.size) {
...
struct pack<std::chrono::system_clock::time_point> {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::chrono::system_clock::time_point& v) const {
        ...
        if ((sec >> 34) == 0) {
            ...
            o.pack_ext(12, -1);
            char buf[12];
            _agpack_store32(&buf[0], static_cast<uint32_t>(nanosec));
            _agpack_store64(&buf[4], sec);
...
struct object_with_zone<std::chrono::system_clock::time_point> {
    void operator()(agpack::object::with_zone& o, const std::chrono::system_clock::time_point& v) const {
        int64_t count = static_cast<int64_t>(v.time_since_epoch().count());
        ...
        if ((sec >> 34) == 0) {
            ...
            char* p = static_cast<char*>(o.zone.allocate_no_align(o.via.ext.size + 1));
            p[0] = static_cast<char>(-1);
            _agpack_store32(&p[1], static_cast<uint32_t>(nanosec));
            _agpack_store64(&p[1 + 4], sec);
            o.via.ext.ptr = p;
        }
    }
};
...
#endif // AGPACK_V1_TYPE_CPP11_CHRONO_HPP
