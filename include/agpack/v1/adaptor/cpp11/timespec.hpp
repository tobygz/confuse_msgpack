...
//
#ifndef AGPACK_V1_TYPE_CPP11_TIMESPEC_HPP
#define AGPACK_V1_TYPE_CPP11_TIMESPEC_HPP
...
struct convert<timespec> {
    agpack::object const& operator()(agpack::object const& o, timespec& v) const {
...
struct pack<timespec> {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const timespec& v) const {
        if ((static_cast<uint64_t>(v.tv_sec) >> 34) == 0) {
            ...
            o.pack_ext(12, -1);
            char buf[12];
            _agpack_store32(&buf[0], static_cast<uint32_t>(v.tv_nsec));
            _agpack_store64(&buf[4], v.tv_sec);
...
struct object_with_zone<timespec> {
    void operator()(agpack::object::with_zone& o, const timespec& v) const {
        if ((static_cast<uint64_t>(v.tv_sec) >> 34) == 0) {
            uint64_t data64 = (static_cast<uint64_t>(v.tv_nsec) << 34) | static_cast<uint64_t>(v.tv_sec);
            if ((data64 & 0xffffffff00000000L) == 0) {
                // timestamp 32
            ...
            char* p = static_cast<char*>(o.zone.allocate_no_align(o.via.ext.size + 1));
            p[0] = static_cast<char>(-1);
            _agpack_store32(&p[1], static_cast<uint32_t>(v.tv_nsec));
            _agpack_store64(&p[1 + 4], v.tv_sec);
            o.via.ext.ptr = p;
        }
    }
};
...
#endif // AGPACK_V1_TYPE_CPP11_TIMESPEC_HPP
