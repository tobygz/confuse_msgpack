...
//
#ifndef AGPACK_V1_TYPE_SIZE_EQUAL_ONLY_HPP
#define AGPACK_V1_TYPE_SIZE_EQUAL_ONLY_HPP

#include "agpack/v1/adaptor/size_equal_only_decl.hpp"
...
struct convert<type::size_equal_only<T> > {
    agpack::object const& operator()(agpack::object const& o, type::size_equal_only<T>& v) const {
        switch(o.type) {
        case agpack::type::ARRAY:
            if (o.via.array.size != agpack::type::size(v.m_t)) throw agpack::type_error();
            break;
        case agpack::type::MAP:
            if (o.via.map.size != agpack::type::size(v.m_t)) throw agpack::type_error();
            break;
        default:
...
struct pack<type::size_equal_only<T> > {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const type::size_equal_only<T>& v) const {
        o << v.m_t;
        return o;
    }
};
...
struct object<type::size_equal_only<T> > {
    void operator()(agpack::object& o, type::size_equal_only<T> const& v) const {
        o << v.m_t;
    }
};
...
struct object_with_zone<type::size_equal_only<T> > {
    void operator()(agpack::object::with_zone& o, type::size_equal_only<T> v) const {
        o << v.m_t;
    }
};
...
#endif // AGPACK_V1_TYPE_SIZE_EQUAL_ONLY_HPP
