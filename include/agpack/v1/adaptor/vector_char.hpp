...
//
#ifndef AGPACK_V1_TYPE_VECTOR_CHAR_HPP
#define AGPACK_V1_TYPE_VECTOR_CHAR_HPP
...
struct convert<std::vector<char, Alloc> > {
    agpack::object const& operator()(agpack::object const& o, std::vector<char, Alloc>& v) const {
        switch (o.type) {
...
struct pack<std::vector<char, Alloc> > {
    ...
    agpack::packer<Stream>& operator()(agpack::packer<Stream>& o, const std::vector<char, Alloc>& v) const {
...
struct object<std::vector<char, Alloc> > {
    void operator()(agpack::object& o, const std::vector<char, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
...
struct object_with_zone<std::vector<char, Alloc> > {
    void operator()(agpack::object::with_zone& o, const std::vector<char, Alloc>& v) const {
        uint32_t size = checked_get_container_size(v.size());
        o.type = agpack::type::BIN;
        o.via.bin.size = size;
        if (size != 0) {
...
#endif // AGPACK_V1_TYPE_VECTOR_CHAR_HPP
