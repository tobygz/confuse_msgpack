...
//
#ifndef AGPACK_CPP11_ZONE_HPP
#define AGPACK_CPP11_ZONE_HPP

#include "agpack/versioning.hpp"
#include "agpack/cpp_config.hpp"
#include "agpack/zone_decl.hpp"
...
private:
    ...
    struct finalizer_array {
        finalizer_array():m_tail(AGPACK_NULLPTR), m_end(AGPACK_NULLPTR), m_array(AGPACK_NULLPTR) {}
        void call() {
        ...
        {
            other.m_tail = AGPACK_NULLPTR;
            other.m_end = AGPACK_NULLPTR;
            other.m_array = AGPACK_NULLPTR;
        }
...
public:
    zone(size_t chunk_size = AGPACK_ZONE_CHUNK_SIZE) noexcept;

public:
    void* allocate_align(size_t size, size_t align = AGPACK_ZONE_ALIGN);
    void* allocate_no_align(size_t size);

    void push_finalizer(void (*func)(void*), void* data);

    template <typename T>
    void push_finalizer(agpack::unique_ptr<T> obj);
...
template <typename T>
inline void zone::push_finalizer(agpack::unique_ptr<T> obj)
{
...
{
    void* x = allocate_align(sizeof(T), AGPACK_ZONE_ALIGNOF(T));
    try {
