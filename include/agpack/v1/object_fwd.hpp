...
#ifndef AGPACK_V1_OBJECT_FWD_HPP
#define AGPACK_V1_OBJECT_FWD_HPP

#include "agpack/object_fwd_decl.hpp"
...
private:
    ...
    static auto check(U*) ->
        ...
        typename std::is_same<
            decltype(adaptor::as<U>()(std::declval<agpack::object>())),
            T
        >::type;
    template <typename...>
    static std::false_type check(...);
public:
    using type = decltype(check<T>(AGPACK_NULLPTR));
...
/**
 * See https://github.com/agpack/agpack-c/wiki/v1_1_cpp_object
 */
struct object {
    union union_type {
        bool boolean;
        ...
        AGPACK_DEPRECATED("please use f64 instead")
        double   dec; // obsolete
#endif // AGPACK_USE_LEGACY_NAME_AS_FLOAT
        double   f64;
        agpack::object_array array;
        agpack::object_map map;
        agpack::object_str str;
        agpack::object_bin bin;
        agpack::object_ext ext;
    };

    agpack::type::object_type type;
    union_type via;
     ...
     */
    bool is_nil() const { return type == agpack::type::NIL; }

#if defined(AGPACK_USE_CPP03)

    /// Get value as T
    /**
     * If the object can't be converted to T, agpack::type_error would be thrown.
     ...
     */
    template <typename T>
    typename std::enable_if<agpack::has_as<T>::value, T>::type as() const;
    ...
    /**
     * If the object is not nil and can't be converted to T, agpack::type_error would be thrown.
     * @tparam T The type of v.
     ...
     */
    template <typename T>
    object(const T& v, agpack::zone& z);

    /// Construct object from T (obsolete)
    /**
     * The object is constructed on the zone `z`.
     * Use `object(const T& v, agpack::zone& z)` instead of this constructor.
     * See https://github.com/agpack/agpack-c/wiki/v1_1_cpp_object
     *
     * @tparam T The type of `v`.
     * @param v The value you want to convert.
     ...
     */
    template <typename T>
    AGPACK_DEPRECATED("please use zone reference version instead of the pointer version")
    object(const T& v, agpack::zone* z);

    template <typename T>
    object& operator=(const T& v);

    operator agpack_object() const;
...
#endif // AGPACK_V1_OBJECT_FWD_HPP
