...
//
#ifndef AGPACK_V2_PARSE_HPP
#define AGPACK_V2_PARSE_HPP
...
public:
    context()
        :m_trail(0), m_cs(AGPACK_CS_HEADER)
    {
    }
...
private:
    ...
    struct array_sv {
        ...
        agpack_container_type type() const { return AGPACK_CT_ARRAY_ITEM; }
    private:
    ...
    struct map_sv {
        ...
        agpack_container_type type() const { return AGPACK_CT_MAP_KEY; }
    private:
        VisitorHolder& m_visitor_holder;
    };
    struct map_ev {
    ...
    struct unpack_stack {
        struct stack_elem {
            stack_elem(agpack_container_type type, uint32_t rest):m_type(type), m_rest(rest) {}
            agpack_container_type m_type;
            uint32_t m_rest;
        };
        unpack_stack() {
            m_stack.reserve(AGPACK_EMBED_STACK_SIZE);
        }
        parse_return push(VisitorHolder& visitor_holder, agpack_container_type type, uint32_t rest) {
...
public:
     ...
     *              See https://github.com/agpack/agpack-c/wiki/v2_0_cpp_visitor#parse-api
...
inline bool parse(const char* data, size_t len, size_t& off, Visitor& v) {
    parse_return ret = agpack::detail::parse_imp(data, len, off, v);
...
inline bool parse(const char* data, size_t len, Visitor& v) {
    std::size_t off = 0;
    return agpack::parse(data, len, off, v);
}
